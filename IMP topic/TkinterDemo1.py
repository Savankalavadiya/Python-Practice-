from tkinter import *
import mysql.connector #write down after insall mysql connector
import tkinter.messagebox as msg # for msg box to write msg


def create_conn(): # my sql>connector> (conn. this method always return connection object)
    return mysql.connector.connect(
            host="localhost", # or write local host ("127.0.0.1",)
            user="root",
            password="",
            database="emp",
        )
...
def insert_data(): #for data insert
    if e_fname.get()=="" or e_lname.get()=="" or e_email.get()=="" or e_mobile.get()=="":
        msg.showinfo("Insert Status","All Fields Are Mandatory")
    else:
        conn=create_conn()
        cursor=conn.cursor()# to work with my sql than use cursor its a laibrary
    ...
    if e_id.get()=="":
        ...
        if row:
            for i in row:
                ...
                msg.showinfo('Sraech Status','No Data Found')
        conn.close()
...
root.resizable(width=False,height=False) #to disble maximize button.
